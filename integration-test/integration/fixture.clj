...
(def default-client-info
  {:name "mcp-clojure-sdk-integration-test"
   :version "1.0.0"})
...
(defn list-prompts-request
  ...
  ([cursor]
   ["prompts/list" (cond-> {} cursor (assoc :cursor cursor))]))

(defn get-prompt-request
  ([name] (get-prompt-request name nil))
  ([name arguments]
   ["prompts/get" (cond-> {:name name}
                    arguments (assoc :arguments arguments))]))
...
(defn set-logging-level-request
  [level]
