...
(defn ^:private receive-message
  ...
  (let [message-type (coercer/input-message-type message)]
    (try (let [response (case message-type
                          (:parse-error :invalid-request)
                            (protocols.endpoint/log client
                                                    :red
                                                    "Error reading message"
                                                    message-type)
                          :request (protocols.endpoint/receive-request client
                                                                       context
                                                                       message)
                          (:response.result :response.error)
                            (protocols.endpoint/receive-response client message)
                          :notification (protocols.endpoint/receive-notification
                                          client
                                          context
                                          message))]
           ;; Ensure client only responds to requests
           (when (identical? :request message-type) response))
         (catch Throwable e
           (protocols.endpoint/log client :red "Error receiving:" e)
           (throw e)))))

(defrecord Client [client-id input output log-ch join request-id sent-requests
                   received-requests received-notifications mock-responses]
  protocols.endpoint/IEndpoint
    (start [this context]
      ...
      (let [pipeline (async/pipeline-blocking
                       ...
                       output
                       ;; TODO: return error until initialize request is
                       ;; received?
                       ;; https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#initialize
                       ;; `keep` means we do not reply to responses and
                       ;; notifications
        ...
        (async/go
          ...
          (async/<! pipeline)
          (deliver join :done)))
      ;; invokers can deref the return of `start` to stay alive until
      ;; server is shut down
      join)
    (shutdown [this]
      ...
      (async/close! log-ch))
    (log [this msg params] (protocols.endpoint/log this :white msg params))
    ...
    (send-request [this method body]
      (let [req (jsonrpc.requests/request (swap! request-id inc) method body)
        ...
        (swap! sent-requests assoc (:id req) {:request p, :start-ns start-ns})
        (async/>!! output req)
        p))
    ...
    (receive-response [this {:keys [id], :as resp}]
      (if-let [{:keys [request start-ns]} (get @sent-requests id)]
        (let [ms (float (/ (- (System/nanoTime) start-ns) 1000000))]
          (protocols.endpoint/log this
                                  :green
                                  (format "received response (%.0fms):" ms)
                                  resp)
          (swap! sent-requests dissoc id)
          (deliver request (if (:error resp) resp (:result resp))))
        (protocols.endpoint/log this
                                :red
...
(defn client
  ...
  (map->Client {:client-id (swap! client-id inc),
                :input (mcp.io-chan/input-stream->input-chan server-out),
                :output (mcp.io-chan/output-stream->output-chan server-in),
...
(defn ^:private await-first-and-remove!
  [client pred coll-type]
      ...
      (if (< tries 20)
        (if-let [elem (first (filter pred @coll*))]
          (do (swap! coll* #(->> %
                                 ...
                                 vec))
              elem)
          (do (Thread/sleep 500) (recur (inc tries))))
        (do (protocols.endpoint/log client :red "timeout waiting:" coll-type)
            (throw (ex-info "timeout waiting for client to receive req/notif"
                            {:coll-type coll-type})))))))

(defn await-server-notification
  [client method]
...
(defn request-and-await-server-response!
  [client method body]
  (let [timeout-ms 180000
        resp (deref (protocols.endpoint/send-request client method body)
                    ...
                    ::timeout)]
    (if (= ::timeout resp)
      (do (protocols.endpoint/log
            client
            :red
            "timeout waiting for server response to client request:"
