...
(defn uri->file [uri] (io/file (URI. uri)))

(defn unescape-uri
  [^String uri]
  (try (URLDecoder/decode uri (.name StandardCharsets/UTF_8))
...
(defn str-includes?
  ...
  (let [s (->> (string/split-lines s)
               (string/join "\n"))
        substr (->> (string/split-lines substr)
                    (string/join "\n"))]
...
(defmacro assert-submaps
  ...
  `(let [maps# ~maps
         res# ~result]
     (and (is (= (count maps#) (count res#))
              (format "Expected %s results, but got: %s \n--\n%s--"
                      (count maps#)
                      (count res#)
                      (with-out-str (pprint/pprint res#))))
          (doseq [[r# m#] (map vector res# maps#)] (assert-submap m# r#)))))
