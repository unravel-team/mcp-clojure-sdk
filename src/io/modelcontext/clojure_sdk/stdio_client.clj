...
(defn start-server-process!
  ...
  [command args & {:keys [env cwd inherit-stderr?]
                   :or {inherit-stderr? true}}]
  (log/trace :fn :start-server-process!
             :command command
             :args args
             :cwd cwd)
...
(defn connect-to-process!
  ...
  (let [input-ch (mcp.io-chan/input-stream->input-chan
                   (.getInputStream process))
        output-ch (mcp.io-chan/output-stream->output-chan
...
(defn stdio-client
  ...
  [client-info command args & {:keys [env cwd inherit-stderr?
                                      roots on-progress on-log
                                      on-resource-updated
  ...
  (let [client (client/create-client client-info
                                     ...
                                     :on-resource-list-changed on-resource-list-changed
                                     :on-tool-list-changed on-tool-list-changed
                                     :on-prompt-list-changed on-prompt-list-changed
                                     :sampling-handler sampling-handler)
        process (start-server-process! command args
                                       :env env
                                       :cwd cwd
...
(defn run!
  ...
  [client-info command args & {:keys [timeout-ms]
                               :or {timeout-ms 30000}
                               :as opts}]
  (log/trace :fn :run!
             :client-info client-info
             :command command)
  (try
    (let [client (apply stdio-client client-info command args
                        (mapcat identity (dissoc opts :timeout-ms)))
          _join (client/start! client)
          init-pending (client/initialize! client)
          init-result (client/deref-or-cancel init-pending timeout-ms ::timeout)]
      (cond
        (= ::timeout init-result)
        (do
          (log/error :fn :run! :error :timeout)
        ...
        (:error init-result)
        (do
          (log/error :fn :run! :error init-result)
          (client/shutdown! client)
